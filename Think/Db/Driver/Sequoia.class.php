...
/**
 * Sequoia数据库驱动 必须配合SequoiaModel使用
 */
class Sequoia extends Db{

    protected $_sequoia           =   null; // SequoiaDb Object
    ...
    public function connect($config='',$linkNum=0) {
        if ( !isset($this->_linkID) ) {
            ...
            $this->_linkID = new \SequoiaDB( $config['hostname'].($config['hostport']?":{$config['hostport']}":''),($config['username']?"{$config['username']}":''),($config['password']?":{$config['password']}@":'')) ;
             //标记连接成功
            $errno = $this->_linkID->getError() ;
    ...
    public function switchCollection($collection,$db='',$master=true){
        ...
        try{
            if(!empty($db)) { // 传入Db则切换数据库
                // 当前SequoiaDb对象
                $this->_dbName  =  $db;
                $this->_sequoia = $this->_linkID->selectCS($db);
            ...
            if($this->_collectionName != $collection) {
                ...
                // 记录开始执行时间
                G('queryStartTime');
                $this->_collection =  $this->_linkID->selectCS('CallRobot')->selectCollection($collection);
                $this->debug();
                $this->_collectionName  = $collection; // 记录当前Collection名称
            }
        }catch (\SequoiaException $e){
            E($e->getMessage());
        }
    }
    ...
    public function command($command=array()) {
        ...
        // 记录开始执行时间
        G('queryStartTime');
        $result   = $this->_sequoia->command($command);
        $this->debug();
        if(!$result['ok']) {
            E($result['errmsg']);
        }
    ...
    public function execute($code,$args=array()) {
        ...
        // 记录开始执行时间
        G('queryStartTime');
        $result   = $this->_sequoia->execute($code,$args);
        $this->debug();
        if($result['ok']) {
    ...
    public function error() {
        $this->error = $this->_sequoia->lastError();
...
//            $result   = $this->_collection->update('{ "$set": { "sentence" : "孙宁测试信息1111111" } }','{_id: {"$oid": "5acf0ff7870d84294a000000"}}');
            $result   = $this->_collection->update($set,json_encode($query));
            $this->debug();
            return $result;
        } catch (\MongoCursorException $e) {
            E($e->getMessage());
    ...
    public function delete($options=array()) {
        ...
        try{
            // 记录开始执行时间
            G('queryStartTime');
            $result   = $this->_collection->remove(json_encode($query));
            $this->debug();
            return $result;
        } catch (\MongoCursorException $e) {
            E($e->getMessage());
    ...
    public function select($options=array()) {
        ...
        N('db_query',1);
        $query  =  $this->parseWhere($options['where']);
        if($query)$query=json_encode($query);

        $field = $this->parseField($options['field']);
        if($field[0])$field=array_fill_keys($field,'');


        try{
            if(C('DB_SQL_LOG')) {
                $this->queryStr   =  $this->_dbName.'.'.$this->_collectionName.'.find(';
                $this->queryStr  .=  $query? $query:'{}';
            ...
            if($options['order']) {
                ...
                $_cursor   = $this->_collection->find($query,$field,$order);
            }
            ...
            if(isset($options['limit'])) {
                ...
                if(!empty($offset)) {
                    ...
                    $_cursor   = $this->_collection->find($query,$field,$order,null,intval($offset));
                }
                if(C('DB_SQL_LOG')) {
...
//                $field = array('password'=>'');
                ...
                $_cursor   = $this->_collection->find($query,$field,$order,null,$offset,$length);
                if( empty( $_cursor ) ) {
                    $err = $this->_linkID -> getError() ;
        ...
        } catch (\SequoiaCursorException $e) {
            E($e->getMessage());
        }
    }
    ...
    public function find($options=array()){
        ...
        if(C('DB_SQL_LOG')) {
            ...
            $this->queryStr .= $query?json_encode($query):'{}';
            $this->queryStr .= $fields?','.json_encode(array_fill_keys($fields,'')):'';
            $this->queryStr .= ')';
        }
        try{
            // 记录开始执行时间
    ...
    public function getTables(){
        ...
        // 记录开始执行时间
        G('queryStartTime');
        $list   = $this->_sequoia->listCollections();
        $this->debug();
        $info =  array();
    ...
    public function parseWhere($where){
        ...
        foreach ($where as $key=>$val){
            if('_id' != $key && 0===strpos($key,'_') && '_string' != $key) {
                // 解析特殊条件表达式
        ...
        if(is_array($val)) {
            if(is_string($val[0])) {
                ...
                if(in_array($con,array('neq','ne','gt','egt','gte','lt','lte','elt'))) { // 比较运算
                    ...
                    $query[$key]=array('$regex'=>$val[1],'$options'=>'i');
